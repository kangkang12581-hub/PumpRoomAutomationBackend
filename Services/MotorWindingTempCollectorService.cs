...
public class MotorWindingTempCollectorService : BackgroundService
{
    private readonly IServiceProvider _serviceProvider;
    private readonly IOpcUaCache _opcUaCache;
    private readonly IOpcUaConnectionManager _connectionManager;
    private readonly ILogger<MotorWindingTempCollectorService> _logger;
    private const string NODE_KEY = "actMotorColiTemp";

    public MotorWindingTempCollectorService(IServiceProvider serviceProvider, IOpcUaCache opcUaCache, 
        IOpcUaConnectionManager connectionManager, ILogger<MotorWindingTempCollectorService> logger)
    {
    ...
    {
        _logger.LogInformation("🚀 启动绕组温度数据采集服务");
        await Task.Delay(TimeSpan.FromSeconds(5), stoppingToken);

        while (!stoppingToken.IsCancellationRequested)
        {
            try { await CollectAndSaveDataAsync(); }
            catch (Exception ex) { _logger.LogError(ex, "❌ 绕组温度数据采集失败"); }
    ...
    private async Task CollectAndSaveDataAsync()
    {
        ...
        var dataService = scope.ServiceProvider.GetRequiredService<IMotorWindingTempService>();
        ...
        foreach (var site in enabledSites)
        {
            try
            {
                ...
                var request = new AddMotorWindingTempRequest
                {
                    SiteId = site.Id,
                    Timestamp = timestamp,
                    Value = value.Value,
                ...
                successCount++;
                _logger.LogInformation("✅ [{SiteCode}] 保存绕组温度: {Value:F3}℃", site.SiteCode, value.Value);
            }
            catch (InvalidOperationException) { }
            catch (Exception ex) { _logger.LogError(ex, "❌ [{SiteCode}] 保存绕组温度失败", site.SiteCode); }
        }

        if (successCount > 0)
            _logger.LogInformation("📊 本次采集: 成功保存 {Count} 个站点的绕组温度数据", successCount);
    }
    ...
    {
        NODE_KEY => "ns=4;s=|var|Inovance-ARM-Linux.Application.GVL_HMI.GHr_actMotorColiTemp",
        _ => string.Empty
    };
}
