...
public class MotorWindingTempService : IMotorWindingTempService
{
    private readonly ApplicationDbContext _context;
    private readonly ILogger<MotorWindingTempService> _logger;

    public MotorWindingTempService(ApplicationDbContext context, ILogger<MotorWindingTempService> logger)
    {
        _context = context;
        _logger = logger;
    }

    public async Task<MotorWindingTemp> AddDataAsync(AddMotorWindingTempRequest request)
    {
        var entity = new MotorWindingTemp
        {
        ...
        try
        {
            _context.MotorWindingTemps.Add(entity);
            await _context.SaveChangesAsync();
            return entity;
        }
        catch (DbUpdateException ex) when (ex.InnerException?.Message.Contains("idx_motorwindingtemp_site_timestamp") == true)
        {
            throw new InvalidOperationException("该时间点的数据已存在");
        }
    }

    public async Task<MotorWindingTempQueryResult> QueryDataAsync(QueryMotorWindingTempRequest request)
    {
        _logger.LogInformation("🔍 查询绕组温度数据: SiteId={SiteId}, StartTime={StartTime}, EndTime={EndTime}, Interval={Interval}", 
        ...
        // 检查数据库中是否有该站点的任何数据
        var totalCount = await _context.MotorWindingTemps.Where(w => w.SiteId == request.SiteId).CountAsync();
        var firstRecord = await _context.MotorWindingTemps.Where(w => w.SiteId == request.SiteId)
            .OrderBy(w => w.Timestamp).Select(w => w.Timestamp).FirstOrDefaultAsync();
        var lastRecord = await _context.MotorWindingTemps.Where(w => w.SiteId == request.SiteId)
        ...
        var result = new MotorWindingTempQueryResult
        {
            SiteId = request.SiteId,
        ...
        _logger.LogInformation("✅ 绕组温度查询结果: 返回 {Count} 条聚合数据", aggregatedData.Count);
    ...
    private async Task<List<AggregatedMotorWindingTempDto>> QueryMinuteDataAsync(QueryMotorWindingTempRequest request)
    {
        var query = _context.MotorWindingTemps
            .Where(f => f.SiteId == request.SiteId && f.Timestamp >= request.StartTime && f.Timestamp < request.EndTime)
            .OrderBy(f => f.Timestamp)
            .Select(f => new AggregatedMotorWindingTempDto
            {
    ...
    private async Task<List<AggregatedMotorWindingTempDto>> QueryAggregatedDataAsync(QueryMotorWindingTempRequest request, string truncateLevel)
    {
        ...
        var sql = $@"
            WITH nearest_records AS (
                SELECT DISTINCT ON (date_trunc('{truncateLevel}', timestamp))
                    ...
                    timestamp,
                    motorwindingtemp as value
                FROM motorwindingtemps
        ...
        var result = new List<AggregatedMotorWindingTempDto>();
        using (var command = _context.Database.GetDbConnection().CreateCommand())
        {
            ...
            using (var reader = await command.ExecuteReaderAsync())
            {
                ...
                {
                    result.Add(new AggregatedMotorWindingTempDto
                    {
    ...
    public async Task<MotorWindingTempDto?> GetLatestDataAsync(int siteId)
    {
        var latest = await _context.MotorWindingTemps.Where(f => f.SiteId == siteId)
            .OrderByDescending(f => f.Timestamp).FirstOrDefaultAsync();
        return latest != null ? new MotorWindingTempDto
        {
            Id = latest.Id,
