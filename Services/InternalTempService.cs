...
public class InternalTempService : IInternalTempService
{
    private readonly ApplicationDbContext _context;
    private readonly ILogger<InternalTempService> _logger;

    public InternalTempService(ApplicationDbContext context, ILogger<InternalTempService> logger)
    {
        _context = context;
        _logger = logger;
    }

    public async Task<InternalTemp> AddDataAsync(AddInternalTempRequest request)
    {
        var entity = new InternalTemp
        {
        ...
        try
        {
            _context.InternalTemps.Add(entity);
            await _context.SaveChangesAsync();
            return entity;
        }
        catch (DbUpdateException ex) when (ex.InnerException?.Message.Contains("idx_internaltemp_site_timestamp") == true)
        {
            throw new InvalidOperationException("该时间点的数据已存在");
        }
    }

    public async Task<InternalTempQueryResult> QueryDataAsync(QueryInternalTempRequest request)
    {
        _logger.LogInformation("🔍 查询柜内温度数据: SiteId={SiteId}, StartTime={StartTime}, EndTime={EndTime}, Interval={Interval}", 
        ...
        // 检查数据库中是否有该站点的任何数据
        var totalCount = await _context.InternalTemps.Where(w => w.SiteId == request.SiteId).CountAsync();
        var firstRecord = await _context.InternalTemps.Where(w => w.SiteId == request.SiteId)
            .OrderBy(w => w.Timestamp).Select(w => w.Timestamp).FirstOrDefaultAsync();
        var lastRecord = await _context.InternalTemps.Where(w => w.SiteId == request.SiteId)
        ...
        var result = new InternalTempQueryResult
        {
            SiteId = request.SiteId,
        ...
        _logger.LogInformation("✅ 柜内温度查询结果: 返回 {Count} 条聚合数据", aggregatedData.Count);
    ...
    private async Task<List<AggregatedInternalTempDto>> QueryMinuteDataAsync(QueryInternalTempRequest request)
    {
        var query = _context.InternalTemps
            .Where(f => f.SiteId == request.SiteId && f.Timestamp >= request.StartTime && f.Timestamp < request.EndTime)
            .OrderBy(f => f.Timestamp)
            .Select(f => new AggregatedInternalTempDto
            {
    ...
    private async Task<List<AggregatedInternalTempDto>> QueryAggregatedDataAsync(QueryInternalTempRequest request, string truncateLevel)
    {
        ...
        var result = new List<AggregatedInternalTempDto>();
        using (var command = _context.Database.GetDbConnection().CreateCommand())
        {
            ...
            using (var reader = await command.ExecuteReaderAsync())
            {
                ...
                {
                    result.Add(new AggregatedInternalTempDto
                    {
    ...
    public async Task<InternalTempDto?> GetLatestDataAsync(int siteId)
    {
        var latest = await _context.InternalTemps.Where(f => f.SiteId == siteId)
            .OrderByDescending(f => f.Timestamp).FirstOrDefaultAsync();
        return latest != null ? new InternalTempDto
        {
            Id = latest.Id,
