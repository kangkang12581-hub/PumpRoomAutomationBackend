...
public class SpeedCollectorService : BackgroundService
{
    private readonly IServiceProvider _serviceProvider;
    private readonly IOpcUaCache _opcUaCache;
    private readonly IOpcUaConnectionManager _connectionManager;
    private readonly ILogger<SpeedCollectorService> _logger;
    private const string NODE_KEY = "actSpeed";

    public SpeedCollectorService(IServiceProvider serviceProvider, IOpcUaCache opcUaCache, 
        IOpcUaConnectionManager connectionManager, ILogger<SpeedCollectorService> logger)
    {
    ...
    {
        _logger.LogInformation("🚀 启动速度数据采集服务");
        await Task.Delay(TimeSpan.FromSeconds(5), stoppingToken);

        while (!stoppingToken.IsCancellationRequested)
        {
            try { await CollectAndSaveDataAsync(); }
            catch (Exception ex) { _logger.LogError(ex, "❌ 速度数据采集失败"); }
    ...
    private async Task CollectAndSaveDataAsync()
    {
        ...
        var dataService = scope.ServiceProvider.GetRequiredService<ISpeedService>();
        ...
        foreach (var site in enabledSites)
        {
            try
            {
                ...
                var request = new AddSpeedRequest
                {
                    SiteId = site.Id,
                    Timestamp = timestamp,
                    Speed = value.Value,
                ...
                successCount++;
                _logger.LogInformation("✅ [{SiteCode}] 保存速度: {Value:F3} rpm", site.SiteCode, value.Value);
            }
            catch (InvalidOperationException) { }
            catch (Exception ex) { _logger.LogError(ex, "❌ [{SiteCode}] 保存速度失败", site.SiteCode); }
        }

        if (successCount > 0)
            _logger.LogInformation("📊 本次采集: 成功保存 {Count} 个站点的速度数据", successCount);
    }
    ...
    {
        NODE_KEY => "ns=4;s=|var|Inovance-ARM-Linux.Application.GVL_HMI.GHr_actFreq",
        _ => string.Empty
    };
}
