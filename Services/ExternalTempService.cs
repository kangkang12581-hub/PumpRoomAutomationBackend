...
public class ExternalTempService : IExternalTempService
{
    private readonly ApplicationDbContext _context;
    private readonly ILogger<ExternalTempService> _logger;

    public ExternalTempService(ApplicationDbContext context, ILogger<ExternalTempService> logger)
    {
        _context = context;
        _logger = logger;
    }

    public async Task<ExternalTemp> AddDataAsync(AddExternalTempRequest request)
    {
        var entity = new ExternalTemp
        {
        ...
        try
        {
            _context.ExternalTemps.Add(entity);
            await _context.SaveChangesAsync();
            return entity;
        }
        catch (DbUpdateException ex) when (ex.InnerException?.Message.Contains("idx_externaltemp_site_timestamp") == true)
        {
            throw new InvalidOperationException("该时间点的数据已存在");
        }
    }

    public async Task<ExternalTempQueryResult> QueryDataAsync(QueryExternalTempRequest request)
    {
        _logger.LogInformation("🔍 查询柜外温度数据: SiteId={SiteId}, StartTime={StartTime}, EndTime={EndTime}, Interval={Interval}", 
        ...
        // 检查数据库中是否有该站点的任何数据
        var totalCount = await _context.ExternalTemps.Where(w => w.SiteId == request.SiteId).CountAsync();
        var firstRecord = await _context.ExternalTemps.Where(w => w.SiteId == request.SiteId)
            .OrderBy(w => w.Timestamp).Select(w => w.Timestamp).FirstOrDefaultAsync();
        var lastRecord = await _context.ExternalTemps.Where(w => w.SiteId == request.SiteId)
        ...
        var result = new ExternalTempQueryResult
        {
            SiteId = request.SiteId,
        ...
        _logger.LogInformation("✅ 柜外温度查询结果: 返回 {Count} 条聚合数据", aggregatedData.Count);
    ...
    private async Task<List<AggregatedExternalTempDto>> QueryMinuteDataAsync(QueryExternalTempRequest request)
    {
        var query = _context.ExternalTemps
            .Where(f => f.SiteId == request.SiteId && f.Timestamp >= request.StartTime && f.Timestamp < request.EndTime)
            .OrderBy(f => f.Timestamp)
            .Select(f => new AggregatedExternalTempDto
            {
    ...
    private async Task<List<AggregatedExternalTempDto>> QueryAggregatedDataAsync(QueryExternalTempRequest request, string truncateLevel)
    {
        ...
        var result = new List<AggregatedExternalTempDto>();
        using (var command = _context.Database.GetDbConnection().CreateCommand())
        {
            ...
            using (var reader = await command.ExecuteReaderAsync())
            {
                ...
                {
                    result.Add(new AggregatedExternalTempDto
                    {
    ...
    public async Task<ExternalTempDto?> GetLatestDataAsync(int siteId)
    {
        var latest = await _context.ExternalTemps.Where(f => f.SiteId == siteId)
            .OrderByDescending(f => f.Timestamp).FirstOrDefaultAsync();
        return latest != null ? new ExternalTempDto
        {
            Id = latest.Id,
