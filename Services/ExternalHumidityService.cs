...
public class ExternalHumidityService : IExternalHumidityService
{
    private readonly ApplicationDbContext _context;
    private readonly ILogger<ExternalHumidityService> _logger;

    public ExternalHumidityService(ApplicationDbContext context, ILogger<ExternalHumidityService> logger)
    {
        _context = context;
        _logger = logger;
    }

    public async Task<ExternalHumidity> AddDataAsync(AddExternalHumidityRequest request)
    {
        var entity = new ExternalHumidity
        {
        ...
        try
        {
            _context.ExternalHumiditys.Add(entity);
            await _context.SaveChangesAsync();
            return entity;
        }
        catch (DbUpdateException ex) when (ex.InnerException?.Message.Contains("idx_externalhumidity_site_timestamp") == true)
        {
            throw new InvalidOperationException("该时间点的数据已存在");
        }
    }

    public async Task<ExternalHumidityQueryResult> QueryDataAsync(QueryExternalHumidityRequest request)
    {
        _logger.LogInformation("🔍 查询柜外湿度数据: SiteId={SiteId}, StartTime={StartTime}, EndTime={EndTime}, Interval={Interval}", 
        ...
        // 检查数据库中是否有该站点的任何数据
        var totalCount = await _context.ExternalHumiditys.Where(w => w.SiteId == request.SiteId).CountAsync();
        var firstRecord = await _context.ExternalHumiditys.Where(w => w.SiteId == request.SiteId)
            .OrderBy(w => w.Timestamp).Select(w => w.Timestamp).FirstOrDefaultAsync();
        var lastRecord = await _context.ExternalHumiditys.Where(w => w.SiteId == request.SiteId)
        ...
        var result = new ExternalHumidityQueryResult
        {
            SiteId = request.SiteId,
        ...
        _logger.LogInformation("✅ 柜外湿度查询结果: 返回 {Count} 条聚合数据", aggregatedData.Count);
    ...
    private async Task<List<AggregatedExternalHumidityDto>> QueryMinuteDataAsync(QueryExternalHumidityRequest request)
    {
        var query = _context.ExternalHumiditys
            .Where(f => f.SiteId == request.SiteId && f.Timestamp >= request.StartTime && f.Timestamp < request.EndTime)
            .OrderBy(f => f.Timestamp)
            .Select(f => new AggregatedExternalHumidityDto
            {
    ...
    private async Task<List<AggregatedExternalHumidityDto>> QueryAggregatedDataAsync(QueryExternalHumidityRequest request, string truncateLevel)
    {
        ...
        var sql = $@"
            WITH nearest_records AS (
                SELECT DISTINCT ON (date_trunc('{truncateLevel}', timestamp))
                    ...
                    timestamp,
                    externalhumidity as value
                FROM externalhumiditys
        ...
        var result = new List<AggregatedExternalHumidityDto>();
        using (var command = _context.Database.GetDbConnection().CreateCommand())
        {
            ...
            using (var reader = await command.ExecuteReaderAsync())
            {
                ...
                {
                    result.Add(new AggregatedExternalHumidityDto
                    {
    ...
    public async Task<ExternalHumidityDto?> GetLatestDataAsync(int siteId)
    {
        var latest = await _context.ExternalHumiditys.Where(f => f.SiteId == siteId)
            .OrderByDescending(f => f.Timestamp).FirstOrDefaultAsync();
        return latest != null ? new ExternalHumidityDto
        {
            Id = latest.Id,
