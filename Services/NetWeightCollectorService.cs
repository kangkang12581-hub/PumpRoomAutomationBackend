...
public class NetWeightCollectorService : BackgroundService
{
    private readonly IServiceProvider _serviceProvider;
    private readonly IOpcUaCache _opcUaCache;
    private readonly IOpcUaConnectionManager _connectionManager;
    private readonly ILogger<NetWeightCollectorService> _logger;
    private const string NODE_KEY = "actNetWeight";

    public NetWeightCollectorService(IServiceProvider serviceProvider, IOpcUaCache opcUaCache, 
        IOpcUaConnectionManager connectionManager, ILogger<NetWeightCollectorService> logger)
    {
    ...
    {
        _logger.LogInformation("🚀 启动净重数据采集服务");
        await Task.Delay(TimeSpan.FromSeconds(5), stoppingToken);

        while (!stoppingToken.IsCancellationRequested)
        {
            try { await CollectAndSaveDataAsync(); }
            catch (Exception ex) { _logger.LogError(ex, "❌ 净重数据采集失败"); }
    ...
    private async Task CollectAndSaveDataAsync()
    {
        ...
        var dataService = scope.ServiceProvider.GetRequiredService<INetWeightService>();
        ...
        foreach (var site in enabledSites)
        {
            try
            {
                ...
                var request = new AddNetWeightRequest
                {
                    SiteId = site.Id,
                    Timestamp = timestamp,
                    Value = value.Value,
                ...
                successCount++;
                _logger.LogInformation("✅ [{SiteCode}] 保存净重: {Value:F3}kg", site.SiteCode, value.Value);
            }
            catch (InvalidOperationException) { }
            catch (Exception ex) { _logger.LogError(ex, "❌ [{SiteCode}] 保存净重失败", site.SiteCode); }
        }

        if (successCount > 0)
            _logger.LogInformation("📊 本次采集: 成功保存 {Count} 个站点的净重数据", successCount);
    }
    ...
    {
        NODE_KEY => "ns=4;s=|var|Inovance-ARM-Linux.Application.GVL_HMI.Ghr_actNetWeight",
        _ => string.Empty
    };
}
